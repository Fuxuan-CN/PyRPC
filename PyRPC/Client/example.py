...
class RPCClient(IRPCClient):
    ...
    def __call__(self, item: str, *args, **kwargs) -> RpcResponse:
        """ 简化调用 RPC 服务 """
        if args:
            return self.call(item, MethodType.FUNCTION, CallType.WITH_ARGS, *args, **kwargs)
        elif kwargs:
            return self.call(item, MethodType.FUNCTION, CallType.WITH_KWARGS, **kwargs)
        elif args and kwargs:
            return self.call(item, MethodType.FUNCTION, CallType.WITH_KEY_VALUE_PAIRS, *args, **kwargs)
        else:
            return self.call(item, MethodType.FUNCTION, CallType.WITH_NO_ARGS)
...
class AsyncRPCClient(RPCClient):
    ...
    async def __call__(self, item: str, *args, **kwargs) -> RpcResponse:
        """ 异步简化调用 RPC 服务 """
        if args:
            return await self.call(item, MethodType.FUNCTION, CallType.WITH_ARGS, *args, **kwargs)
        elif kwargs:
            return await self.call(item, MethodType.FUNCTION, CallType.WITH_KWARGS, **kwargs)
        elif args and kwargs:
            return await self.call(item, MethodType.FUNCTION, CallType.WITH_KEY_VALUE_PAIRS, *args, **kwargs)
        else:
            return await self.call(item, MethodType.FUNCTION, CallType.WITH_NO_ARGS)
