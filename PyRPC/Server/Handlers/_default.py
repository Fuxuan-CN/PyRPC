...
class DefaultHandler(IRPCHandler):
        ...
        try:
            if request.callbody.callitem in self.call_items:
                if request.callType == CallType.WITH_NO_ARGS:
                    result = self.call_function(self.call_items[request.callbody.callitem])
                elif request.callType == CallType.WITH_ARGS:
                    result = self.call_function(self.call_items[request.callbody.callitem], *request.callbody.args)
                elif request.callType == CallType.WITH_KWARGS:
                    result = self.call_function(self.call_items[request.callbody.callitem], **request.callbody.kwargs)
                elif request.callType == CallType.WITH_KEY_VALUE_PAIRS:
                    result = self.call_function(self.call_items[request.callbody.callitem], *request.callbody.args, **request.callbody.kwargs)
                else:
            ...
            return self.exception_returned(e)
    ...
    def call_method(self, obj: object, method_name: str, *args, **kwargs) -> Any:
        """ 处理方法调用 """
    ...
    def exception_returned(self, exception: Exception) -> None:
        """ 返回异常信息 """
